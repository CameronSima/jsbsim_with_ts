...
#include "FGFDMExec.h"
// #include "input_output/FGUDPInputSocket.h" // Disabled for WebAssembly
...
bool FGInput::Load(Element* el)
{
  ...
  if (type.empty() || type == "SOCKET") {
    cerr << "ERROR: Socket input not supported in WebAssembly build" << endl;
    return false;
  } else if (type == "QTJSBSIM") {
    cerr << "ERROR: UDP socket input not supported in WebAssembly build" << endl;
    return false;
