...
// #include "input_output/FGOutputFG.h" // Disabled for WebAssembly
                    ...
                    std::vector<SGPropertyNode_ptr> & outputProperties)
{
  ...
  if (type == "CSV") {
    ...
    cerr << "ERROR: Socket output not supported in WebAssembly build" << endl;
    return false;
  } else if (type == "FLIGHTGEAR") {
    cerr << "ERROR: FlightGear output not supported in WebAssembly build" << endl;
    return false;
